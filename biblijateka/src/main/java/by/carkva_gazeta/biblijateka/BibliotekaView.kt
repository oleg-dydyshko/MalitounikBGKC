        ...
        override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
            ...
            if (arrayList[position].size == 3) {
                ...
                viewHolder.buttonPopup?.let {
                    viewHolder.buttonPopup?.setOnClickListener { showPopupMenu(it, position, arrayList[position][0]) }
                }
                ...
                if (file.exists()) {
                    Runnable {
                        val options = BitmapFactory.Options()
                        options.inPreferredConfig = Bitmap.Config.ARGB_8888
                        val bitmap = BitmapFactory.decodeFile("$filesDir/image_temp/" + arrayList[position][5].substring(t1 + 1), options)
                        runOnUiThread {
                            viewHolder.imageView?.setImageBitmap(bitmap)
                            viewHolder.imageView?.visibility = View.VISIBLE
                        }
