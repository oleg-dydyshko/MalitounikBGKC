...
class PinchZoomRecyclerView : RecyclerView {
    ...
    private inner class ScaleListener : ScaleGestureDetector.SimpleOnScaleGestureListener() {
        override fun onScale(detector: ScaleGestureDetector): Boolean {
            val scaleFactor = 1f.coerceAtLeast((mScaleFactor * detector.scaleFactor).coerceAtMost(mMaxZoom))
    ...
    private fun clampPosition() {
        ...
        mPosX = maxPosX.coerceAtMost((-maxPosX).coerceAtLeast(mPosX))
        mPosY = maxPosY.coerceAtMost((-maxPosY).coerceAtLeast(mPosY))
