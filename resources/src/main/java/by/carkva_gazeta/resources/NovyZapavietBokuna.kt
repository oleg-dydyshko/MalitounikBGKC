...
class NovyZapavietBokuna : BaseActivity(), DialogFontSizeListener, DialogBibleRazdelListener, BibleListiner, DialogBibleNatatka.DialogBibleNatatkaListiner, DialogAddZakladka.DialogAddZakladkiListiner, DialogHelpFullScreenSettings.DialogHelpFullScreenSettingsListener {
    ...
    override fun onPause() {
        ...
        prefEditors.putBoolean("bible_time_bokuna_zavet", true)
        prefEditors.putInt("bible_time_bokuna_kniga", kniga)
        prefEditors.putInt("bible_time_bokuna_glava", binding.pager.currentItem)
        prefEditors.putInt("bible_time_bokuna_stix", fierstPosition)
        prefEditors.apply()
        val gson = Gson()
        clearEmptyPosition()
        ...
        val listFiles = File("$filesDir/BibliaBokunaNovyZavet").listFiles()
        listFiles?.forEach {
        ...
        val file = File("$filesDir/BibliaBokunaNovyZavet/$kniga.json")
    ...
    override fun addZakladka(color: Int) {
        val fragment = supportFragmentManager.findFragmentByTag("f" + binding.pager.currentItem) as NovyZapavietBokunaFragment
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        title = resources.getStringArray(R.array.bokunan)[kniga]
        ...
        binding.pager.registerOnPageChangeCallback(object : ViewPager2.OnPageChangeCallback() {
            override fun onPageSelected(position: Int) {
                ...
                men = DialogVybranoeBibleList.checkVybranoe(kniga, position, DialogVybranoeBibleList.PEREVODBOKUNA)
                if (glava != position) fierstPosition = 0
                invalidateOptionsMenu()
            }
        })
        men = DialogVybranoeBibleList.checkVybranoe(kniga, glava, DialogVybranoeBibleList.PEREVODBOKUNA)
    ...
    override fun onMenuItemSelected(item: MenuItem): Boolean {
        ...
        if (id == R.id.action_vybranoe) {
            men = DialogVybranoeBibleList.setVybranoe(title, kniga, BibleGlobalList.mListGlava, DialogVybranoeBibleList.PEREVODBOKUNA, true)
            if (men) {
                MainActivity.toastView(this, getString(R.string.addVybranoe))
                if (!DialogVybranoeBibleList.checkVybranoe(DialogVybranoeBibleList.PEREVODBOKUNA)) {
                    MenuVybranoe.vybranoe.add(0, VybranoeData(Bogashlugbovya.vybranoeIndex(), DialogVybranoeBibleList.PEREVODBOKUNA, getString(R.string.title_biblia_bokun)))
                    val gson = Gson()
    ...
    private inner class MyPagerAdapter(activity: FragmentActivity) : FragmentStateAdapter(activity) {
        ...
        override fun onBindViewHolder(holder: FragmentViewHolder, position: Int, payloads: MutableList<Any>) {
            ...
            val fragment = supportFragmentManager.findFragmentByTag("f" + holder.itemId) as? NovyZapavietBokunaFragment
            fragment?.upDateListView()
        }

        override fun getItemCount() = fullglav

        override fun createFragment(position: Int): NovyZapavietBokunaFragment {
            val styx = if (glava != position) 0
            else fierstPosition
            return NovyZapavietBokunaFragment.newInstance(title, position, kniga, styx)
        }
    }
}