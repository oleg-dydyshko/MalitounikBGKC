...
class NovyZapavietCarniauski : BaseActivity(), DialogFontSizeListener, DialogBibleRazdelListener, BibleListiner, DialogBibleNatatka.DialogBibleNatatkaListiner, DialogAddZakladka.DialogAddZakladkiListiner, DialogHelpFullScreenSettings.DialogHelpFullScreenSettingsListener {
    ...
    override fun onPause() {
        ...
        prefEditors.putBoolean("bible_time_carniauski_zavet", true)
        prefEditors.putInt("bible_time_carniauski_kniga", kniga)
        prefEditors.putInt("bible_time_carniauski_glava", binding.pager.currentItem)
        prefEditors.putInt("bible_time_carniauski_stix", fierstPosition)
        prefEditors.apply()
        val gson = Gson()
        clearEmptyPosition()
        ...
        val listFiles = File("$filesDir/BibliaCarniauskiNovyZavet").listFiles()
        listFiles?.forEach {
        ...
        val file = File("$filesDir/BibliaCarniauskiNovyZavet/$kniga.json")
    ...
    override fun addZakladka(color: Int) {
        val fragment = supportFragmentManager.findFragmentByTag("f" + binding.pager.currentItem) as NovyZapavietCarniauskiFragment
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        title = resources.getStringArray(R.array.charniauskin)[kniga]
        ...
        binding.pager.registerOnPageChangeCallback(object : ViewPager2.OnPageChangeCallback() {
            override fun onPageSelected(position: Int) {
                ...
                men = DialogVybranoeBibleList.checkVybranoe(kniga, position, DialogVybranoeBibleList.PEREVODCARNIAUSKI)
                if (glava != position) fierstPosition = 0
                invalidateOptionsMenu()
            }
        })
        men = DialogVybranoeBibleList.checkVybranoe(kniga, glava, DialogVybranoeBibleList.PEREVODCARNIAUSKI)
        ...
        binding.titleToolbar.text = savedInstanceState?.getString("title") ?: getString(R.string.novy_zapaviet)
    }
    ...
    override fun onBack() {
        when {
            paralel -> {
                ...
                binding.titleToolbar.text = getString(R.string.novy_zapaviet)
                binding.subtitleToolbar.text = title
                paralel = false
                invalidateOptionsMenu()
            }
    ...
    override fun onMenuItemSelected(item: MenuItem): Boolean {
        ...
        if (id == R.id.action_vybranoe) {
            men = DialogVybranoeBibleList.setVybranoe(title, kniga, BibleGlobalList.mListGlava, DialogVybranoeBibleList.PEREVODCARNIAUSKI,  true)
            if (men) {
                MainActivity.toastView(this, getString(R.string.addVybranoe))
                if (!DialogVybranoeBibleList.checkVybranoe(DialogVybranoeBibleList.PEREVODCARNIAUSKI)) {
                    MenuVybranoe.vybranoe.add(0, VybranoeData(Bogashlugbovya.vybranoeIndex(), DialogVybranoeBibleList.PEREVODCARNIAUSKI, getString(R.string.title_biblia_charniauski)))
                    val gson = Gson()
    ...
    override fun setOnClic(cytanneParalelnye: String, cytanneSours: String) {
        ...
        binding.conteiner.text = pm.paralel(this.cytanneParalelnye, DialogVybranoeBibleList.PEREVODCARNIAUSKI).trim()
    ...
    private inner class MyPagerAdapter(activity: FragmentActivity) : FragmentStateAdapter(activity) {
        ...
        override fun onBindViewHolder(holder: FragmentViewHolder, position: Int, payloads: MutableList<Any>) {
            ...
            val fragment = supportFragmentManager.findFragmentByTag("f" + holder.itemId) as? NovyZapavietCarniauskiFragment
            fragment?.upDateListView()
        }

        override fun getItemCount() = fullglav

        override fun createFragment(position: Int): NovyZapavietCarniauskiFragment {
            val styx = if (glava != position) 0
            else fierstPosition
            return NovyZapavietCarniauskiFragment.newInstance(title, position, kniga, styx)
        }
    }
}