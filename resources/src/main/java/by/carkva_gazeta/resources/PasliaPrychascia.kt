...
class PasliaPrychascia : AppCompatActivity(), OnTouchListener, DialogFontSizeListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        TextView.setTextSize(TypedValue.COMPLEX_UNIT_SP, fontBiblia)
        val r = this@PasliaPrychascia.resources
        var inputStream = r.openRawResource(raw.paslia_prychascia1)
        when (intent.extras?.getInt("paslia_prychascia", 1) ?: 1) {
            1 -> {
                inputStream = r.openRawResource(raw.paslia_prychascia1)
                resurs = "paslia_prychascia1"
                title = "Малітва падзякі"
            }
            2 -> {
                inputStream = r.openRawResource(raw.paslia_prychascia2)
                resurs = "paslia_prychascia2"
                title = "Малітва сьв. Васіля Вялікага"
            }
            3 -> {
                inputStream = r.openRawResource(raw.paslia_prychascia3)
                resurs = "paslia_prychascia3"
                title = "Малітва Сымона Мэтафраста"
            }
            4 -> {
                inputStream = r.openRawResource(raw.paslia_prychascia4)
                resurs = "paslia_prychascia4"
                title = "Iншая малітва"
            }
            5 -> {
                inputStream = r.openRawResource(raw.paslia_prychascia5)
