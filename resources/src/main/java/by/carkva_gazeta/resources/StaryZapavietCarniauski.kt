...
class StaryZapavietCarniauski : BaseActivity(), DialogFontSizeListener, DialogBibleRazdelListener, BibleListiner, DialogBibleNatatka.DialogBibleNatatkaListiner, DialogAddZakladka.DialogAddZakladkiListiner, DialogHelpFullScreenSettings.DialogHelpFullScreenSettingsListener {
    ...
    override fun onPause() {
        ...
        prefEditors.putBoolean("bible_time_carniauski_zavet", false)
        ...
        val listFiles = File("$filesDir/BibliaCarniauskiStaryZavet").listFiles()
        listFiles?.forEach {
        ...
        val file = File("$filesDir/BibliaCarniauskiStaryZavet/$kniga.json")
    ...
    override fun addZakladka(color: Int) {
        val fragment = supportFragmentManager.findFragmentByTag("f" + binding.pager.currentItem) as StaryZapavietCarniauskiFragment
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        title = resources.getStringArray(R.array.charniauskis)[kniga]
    ...
    private fun setTollbarTheme() {
        ...
        binding.subtitleToolbar.text = getString(R.string.title_biblia_charniauski2)
        if (dzenNoch) {
    ...
    override fun onMenuItemSelected(item: MenuItem): Boolean {
        ...
        if (id == R.id.action_vybranoe) {
            men = DialogVybranoeBibleList.setVybranoe(title, kniga, BibleGlobalList.mListGlava, DialogVybranoeBibleList.PEREVODCARNIAUSKI)
            if (men) {
    ...
    private inner class MyPagerAdapter(activity: FragmentActivity) : FragmentStateAdapter(activity) {
        ...
        override fun onBindViewHolder(holder: FragmentViewHolder, position: Int, payloads: MutableList<Any>) {
            ...
            val fragment = supportFragmentManager.findFragmentByTag("f" + holder.itemId) as? StaryZapavietCarniauskiFragment
            fragment?.upDateListView()
        }

        override fun getItemCount() = fullglav

        override fun createFragment(position: Int): StaryZapavietCarniauskiFragment {
            val styx = if (glava != position) 0
            else fierstPosition
            return StaryZapavietCarniauskiFragment.newInstance(title, position, kniga, styx)
        }
    }
}