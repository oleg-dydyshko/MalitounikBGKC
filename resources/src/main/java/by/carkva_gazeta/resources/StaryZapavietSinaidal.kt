...
class StaryZapavietSinaidal : BaseActivity(), DialogFontSizeListener, DialogBibleRazdelListener, BibleListiner, DialogBibleNatatka.DialogBibleNatatkaListiner, DialogAddZakladka.DialogAddZakladkiListiner, DialogHelpFullScreenSettings.DialogHelpFullScreenSettingsListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        TabLayoutMediator(binding.tabLayout, binding.pager, false) { tab, position ->
            tab.text = if (kniga == 21) getString(R.string.psinaidal) + " " + (position + 1)
            else getString(R.string.rsinaidal) + " " + (position + 1)
        }.attach()
        ...
        binding.titleToolbar.text = savedInstanceState?.getString("title") ?: getText(R.string.stsinaidal)
    }
    ...
    override fun onBack() {
        when {
            paralel -> {
                ...
                binding.titleToolbar.text = resources.getText(R.string.stsinaidal)
                binding.subtitleToolbar.text = title
                paralel = false
                invalidateOptionsMenu()
            }
    ...
    override fun onMenuItemSelected(item: MenuItem): Boolean {
        ...
        if (id == R.id.action_vybranoe) {
            men = DialogVybranoeBibleList.setVybranoe(title, kniga, BibleGlobalList.mListGlava, DialogVybranoeBibleList.PEREVODSINOIDAL)
            if (men) {
